<template>
  ...
  <div v-if="savedRecipes.length === 0" class="no-recipes">
    <p style="color: white;">No tienes recetas guardadas de platos fuertes todavía.</p>
  </div>
  </div>
  </template>
  ...
  savedRecipes.value = response.data.filter(
    (recipe) => recipe.recipe_type === 'platos_fuertes'
  );
  ...
  const section = document.querySelector('.guardadoPlatos-page'); // Cambia al contenedor específico del componente
  if (section) {
