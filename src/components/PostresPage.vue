    ...
    <div class="recipes-container">
      ...
      >
        ...
        <div class="recipe-info">
          ...
          <button @click="selectRecipe(recipe)" style="margin-left: 60px; margin-bottom: 0px;">Ver Receta</button>
        </div>
      </div>
    </div>
...
const fetchRecipes = async () => {
  try {
    const response = await api.get('/postres'); // Endpoint real
    recipes.value = response.data.map((recipe) => ({
      ...recipe,
      recipe_type: 'postres', // Asegura que el tipo de receta esté presente
    }));
  } catch (error) {
