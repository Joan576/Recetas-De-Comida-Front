<template>
  <div class="recetas-page" style="margin-bottom: 21px;">
    ...
    <h1 class="title" style="margin-top: 30px;">Platos Fuertes</h1>
    ...
    <div class="recipes-container">
      ...
      >
        ...
        <div class="recipe-info">
          ...
          <button @click="selectRecipe(recipe)" style="margin-left: 62px; margin-bottom: 0px;">Ver Receta</button>
        </div>
      </div>
    </div>
...
const fetchRecipes = async () => {
  try {
    const response = await api.get('/platos-fuertes'); // Endpoint real
    recipes.value = response.data.map((recipe) => ({
      ...recipe,
      recipe_type: 'platos_fuertes', // Asegura que el tipo de receta esté presente
    }));
  } catch (error) {
