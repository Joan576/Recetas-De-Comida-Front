<template>
    <div class="guardadoPostres-page">
      <hr>
      <h1>Postres</h1>
  ...
  <div v-if="savedRecipes.length === 0" class="no-recipes">
    <p style="color: white;">No tienes recetas guardadas de postres todavía.</p>
  </div>
  </div>
  </template>
  ...
  savedRecipes.value = response.data.filter(
    (recipe) => recipe.recipe_type === 'postres'
  );
  ...
  const section = document.querySelector('.guardadoPostres-page'); // Cambia al contenedor específico del componente
  if (section) {
