<template>
  <div class="guardado-page">
    ...
    <div v-if="savedRecipes.length === 0" class="no-recipes">
      <p style="color: white;">No tienes recetas guardadas de entradas todavía.</p>
    </div>
  </div>
</template>
...
const fetchSavedRecipes = async () => {
  ...
  try {
    ...
    savedRecipes.value = response.data.filter(
      (recipe) => recipe.recipe_type === 'entradas'
    );
...
const closeRecipeDetails = () => {
  const section = document.querySelector('.saved-recipes-container');
  if (section) {
    window.scrollTo({
      top: section.offsetTop - 150, // Ajusta este valor para desplazar más hacia arriba
      behavior: 'smooth',
    });
  }
